 ...
 * 食物来源（食物库）。
 *
 * 与 [ExerciseSource] **刻意分开**：动作侧有 `AI_SUGGESTED` 这一态（AI 可以直接产出一个新动作），
 * 而食物侧 AI **只给建议、不产生库条目**（见 `.scratch/ironhabit-diet-food-log/spec.md` §1），
 * 所以这里只有两态。硬套 `ExerciseSource` 会留下一个永远不该出现的第三态。
 */
