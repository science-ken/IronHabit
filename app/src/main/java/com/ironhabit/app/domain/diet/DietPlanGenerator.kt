...
object DietPlanGenerator {
     ...
     *
     * ## 宏量口径（**近似，诚实登记**）
     * 热量 / 蛋白按 **(保留条目数 / 原条目数)** 等比缩放，取整次序为
     * **「以未取整的目标基值 × 保留比，最后再 `roundToInt`」** —— 即
     * `round(targetKcal × ratio × factor)`，**不是**「先把目标基值取整、再乘保留比」。
     * （两者可能相差 1：目标基值 1670×0.25 = 417.5 不先取整，保留 2/3 → 417.5×2/3 = 278.33 → **278**；
     * 若先把基值取整成 418 再乘 → 279。本口径得 278，未取整更接近真实值，**有意为之，勿当 bug 改**。）
