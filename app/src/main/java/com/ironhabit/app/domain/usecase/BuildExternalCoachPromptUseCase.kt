...
8. 如果我在下面【我的数据】里找不到 library 数组（那是我能用的动作清单），**不要**只回一个空壳计划：请在 analysis 里直接说"没收到 library"，并告诉用户重新复制模板整段再发一次。
