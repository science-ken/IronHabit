...
- **A13 已修**（习惯「每周指定日」全不选不再静默变全周）：`needsWeeklyDays` 是**派生**判据，
  保存按钮据此禁用 + chip 行下出红字，VM 里那句 `?: WEEKLY_DAYS_ALL` 删掉换成同判据守卫。
  真机截图 `shots/rw-a13.png`：七个 chip 全白、保存那颗是灰的、点它表单不关、库里掩码一字未动。
  （这轮开头我把它和「每周相同」当成同一件事问过，用户答的是后者 —— 所以这条当时确实没定，不是漏做。）
- **D7 已收口**（`61b952c`）：磁贴与弹层标题从「今日习惯」改成「习惯」，**分母不动** ——
  按应做日过滤就没法提前或额外勾一天，能力换口径不值。周五显示 0/1 现在是真话。
- **「insight 按周缓存」这条待办不存在**（AI 教练阶段 5 那份记录写的），两处前提都错：insight 用的是
  滚动 14 天窗口不按周；而 `AiCoachViewModel` 在 Tab 切换中**存活**（真机：翻到上一周 → 切走 → 切回来，
  周游标还在「上一周 · 9/14~9/20」，`weekOffset` 只活在 VM 里）。我为此写的 `CoachInsightHolder`
  一版改动（含 2 条单测）**已全部撤销**。别再排。
- ⚠️ **今日页不再有"把整周变成永久模板"的入口** —— 只剩训练页「每周都加（以后每周都用这份）」。
  这是这次拍板的直接后果，不是漏做。

- **内置食物库 29 → 127 条**（`1a35528`）。不是接数据集（没有合法可再分发的中文成分表，见 D3 勘误），
  是按已批准的设计**手写**：乳类/坚果种子/油脂/带皮禽肉与下水/鱼虾蟹/蔬菜/水果/杂粮主食/成品菜。
  忌口标签里 **SPICY 以前全库 0 条** —— 那条过滤从来没被测过，现在有大蒜和辣椒。
  判据只有 Atwater 能量自洽 ≤12%（`BuiltInFoodsDataTest`），并且**故意改错一条验过它会红**。
  ⚠️ 这批数值是手抄的，成品菜那 5 条最容易错，**待用户抽查**。

- **食物库改成两列一排**（`458077d`）：127 条按旧版一条三行要滑 36 屏（真机量到一条 348px），
  现在挑选屏一屏 ~9 条、管理屏 ~7 条。**卡片压不到更矮的真正原因是 48dp 最小触摸高度**（B2 那条无障碍修复定的线），
  不是没想办法 —— 要再密只能挪走热量（chip 流）或加 `category` 列按类分排（后者是迁移 + 备份 v6 + 逐条标类）。
  ⚠️ 预览原型里"一屏 ~16 条"是我估错的，实测 9 条，原型已改正。
- - **D5 的荒谬界加了**（`InputLimits.isValidHabitTarget` = `isFinite && 0..1e6`），契约测试里那条
  "欠账"豁免随之删掉 —— **`exemptions` 现在是空的，以后要加前先想清楚是不是真该豁免**。
  上界刻意放到 100 万：这一栏没有单位，编一个"合理"的紧数会把合法值锁死，比不修更糟。
  真机填 `-4` 保存 → snackbar 挡住、库里没这条。**仍归用户的**是按单位的合理区间。
- **E2 不是缺口**：动作库/食物库的中文搜索逻辑早有 JVM 测试钉住（`硬`→硬拉、`肱三头`→命中次要肌），
  这台机缺的只是往输入框里打中文的能力。
