...
## ✅ 最新快照（2026-09-22 白天收工 · HEAD `74279ac` · **21 个本地 commit 未 push、未打 tag**）

下面那份 09-20 的快照描述的是**远端**状态（origin/main = `3919aa8`，版本 20 / 2.0.9）；
本地在它之上已经堆了 21 个 commit（38 文件 / +1494 / -235），**版本号一直没动**，
所以"设备上是 2.0.9"和"代码是最新的"这两件事同时成立，别拿 versionCode 判断新旧。

这一轮做完的事、每条的真机读数、以及新查出的 D14/D15/D16，全部记在
**`docs/FIX-PLAN-2026-09-21.md` 末尾「第 10~12 刀 + 第 4/2/6/10 行验证」**那一节；
缺陷的原始现象/证据在 `.scratch/defects.md`（未被 git 跟踪）。一句话版：

- 修了 3 刀：食物库停用不再是单向门（`6e7ece0`）、「已保留 N 条」只数界面上找得着的行（`d29c6f0`）、
  勾「每周相同」不再清空模板行的手改保护标记（`74279ac`，这条是测出来的新 bug）。
- 验了 4 条此前没人走过的路径：预览页「全部采纳」、「每周相同」开关、断网回落本地规则、**导入备份往返**。
- 607 个单测全绿（`--rerun-tasks`）；收工重新拉库比对，全库与开工基线 md5 一致。
...
| 事项 | 代价 | 备注 |
|---|---|---|
| D15 预览页不区分「联网失败回落」与"本来就本地" | 20~30min | 要拍板：`ai_source_fallback` 那条现成的串要不要搬到预览页 |
| D16 导入会改写 `created_at = 0` 的行 | 30min | 判据应改成 `schemaVersion < 3`，不是"值是否为 0" |
| #10-2 导出「保存到本地」SAF 面板 | 1.5h | 用户已定：先不做，等真机 |
| #9-2 习惯行 emoji 是否兼作勾选 | 2h | 设计改动，要先出对比图给用户拍 |
| 第 9/11/12 行零碎未测项 | 约 1h | 新建编辑食物、身体数据新增、打卡历史展开、动作详情→编辑往返 |
| 第 13 行 动作库中文搜索 | — | `adb shell input text` 打不进中文，需用户手点或装 ADBKeyboard |
| 第 5 行 #10-1 分享兜底 | — | MuMu 上系统自己吞了 chooser 不抛异常，`try/catch` 分支永远走不到，只能换真机 |
