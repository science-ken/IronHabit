...
## ✅ 最新快照（2026-09-22 晚 · 合并待办的第 1~5 步做完 · 每步各推一次）
...
第 4 步 A7+A4+DST（`2efb42e`）、第 5 步 A9+C1/C5 与 C4 驳回（本刀）。
**下一步是第 6 步 C2+C3**（streak 数字放大 + 状态提示装进容器），要先出效果图再动。
...
624 个单测全绿（`--rerun-tasks`）；深浅两套目测各拍两张（`shots/b5-*.png`）；
