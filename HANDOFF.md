...
七个本地 commit（**都还没推**，`origin/main` 落后 7）：
`2b34307` 数据层 → `a83cb04` 档案卡 + 四联 → `2b548b0` 新路由「训练统计」 → `36be3a1` 记录台账 + 存哪儿
→ `ea29194` 饼图同色系色阶 + `formatMonthDay` 收拢 → `f461ab0` 身体数据页重排（J3）→ 本条文档。
...
**还剩的**：骨架屏只做到"块数与高度近似同构"（C6–C12 第 ⑦ 条的定制同构没做）；
`versionCode` 仍是 20 / 2.0.9，这一轮全部是 debug 装机验证的。
