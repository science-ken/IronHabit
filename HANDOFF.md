...
1. **你的源码在这里**（免 git，直接可编译）：
   ...
   （已同步到 `versionCode = 17` / `versionName = "2.0.6"`；如果你的沙箱只能访问别的目录，把整个文件夹拷过去即可，它是纯源码、不含 `.git`）
...
| 远端 HEAD | `origin/main` = P0 修复提交；最新 tag `v2.0.6`（已出**签名 release APK**） |
| 版本 | `versionCode = 17` / `versionName = "2.0.6"` |
| 当前 APK | GitHub Release `v2.0.6` 的 `app-release.apk`（**签名**，CI 自动产出） |
