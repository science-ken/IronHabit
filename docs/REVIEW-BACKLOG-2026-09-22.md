...
| D7 | **「今日习惯」的分母不看"今天该不该做"** —— 2026-09-23 真机验过，不是推演：把 `喝水` 设成只排周三（存成 `mask=68`；顺带说一句，"周日关不掉"是我上一轮的**误报**，见 A12 里的撤回），拨到**周五 9/25** → 磁贴写 **「今日习惯 0/1」**，点开弹层 `喝水` 照样列在里面等着打勾。截图 `shots/D5-fri-925.png`、`shots/D5-fri-sheet.png`。<br>同一屏的「本周复盘」热力条里 **五 是灰的** —— app 自己知道周五不是它的日子，只有习惯分母不知道。<br>代码位置：`TodayBento.kt:78` `habitTotal = state.habits.size`，来源 `GetTodayOverviewUseCase.kt:47 observeActiveHabits()`（**全量启用习惯，无应做日过滤**）；而连续天数那条链路**是过滤的**（`Habit.expectedWeekdays` 喂 `StreakCalculator`）。同一个习惯，两套口径。<br>⚠️ **这是口径决定不是 bug**：按应做日过滤，用户就没法提前/额外勾一天；不过滤，「今日」两个字就永远等于"所有习惯"。**等你定** | 要你定 |
