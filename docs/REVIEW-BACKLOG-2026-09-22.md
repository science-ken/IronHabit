...
| ~~C6–C12~~ | ~~「我的」页 7 处重构：档案卡加完整度环、**新增关键指标四宫格**（这页现在一个数字都没有）、入口行加图标+副标题、入口分两组去掉手工 Divider、趋势图装卡片+周期切换、饼图修布局+中心总数+换色、骨架屏与真实结构同构~~ **已做（`2b34307`…`36be3a1`，按方案 J 落地，见 `HANDOFF.md` 顶部）**：<br>① 完整度环 ✅ —— 但分母是**代码真判据 6 项**（性别/年龄/身高/体脂/目标体重/器械），不是报告估的 5；<br>② 四宫格 ✅ 连续·本周出勤·累计记录·体重，口径全部与今日页同源；<br>③ 入口带图标+副标题 ✅ 四行「记录台账」，右侧是真条数；<br>④ 分两组去 Divider ◐ 改成「台账卡 + 存哪儿卡」两块容器，卡内仍有分隔线（不是原提案那种"两组入口列表"的形态）；<br>⑤ 趋势图装卡片 + 周期切换 ✅ 在**新路由「训练统计」**里，7/30/90 三档，标题不再自带「近 30 天」；<br>⑥ 饼图 ◐ 已改环形 + 中心总次数，**分类色没换成同色系明度阶梯**（浅底 `primary + secondary` 实测只差 1.16:1，两块楔形只靠色相分）；<br>⑦ 骨架屏 ◐ 首屏两块用 64dp 的 `LoadingSkeleton` 近似同构，没有档案卡/四联各自的定制骨架 | 约 300 行这条估得偏低：实际动了 20 个文件，含 4 条新聚合 SQL + 1 条新路由。报告说"与其它四个 Tab 不同源竞争"这点我核过是对的（`ProfileScreen` 不喂 AI context） |

### 这一轮之后新挂的两条

| # | 事项 | 代价 |
|---|---|---|
| P1 | 饼图分类色改同色系明度阶梯（上面 ⑥ 没做完的后半）：浅底第 2 档 `secondary` 灰与 `primary` 只差 1.16:1，两块楔形只靠色相分；深底方向还是反的 | 20min + 深浅各一张截图 |
| P2 | `formatMonthDay` 还散在四个页面里各一份私有副本（`BodyMetricsScreen` / `ExerciseDetailScreen` / `HistoryScreen` / `TrainScreen`），逐字相同。本包 `PlanDateStrip` 那份已在 J 第 4 刀收拢，剩下四份收进 `ui/components/TrendChart.kt` 那个 internal 版本 | 20min，纯收拢、零行为变化 |
